...
__global__ void update_flake_kernel(int *flake, int *new_flake, int *rule) {
    ...
    int flake_idx = (col * n_rows + row) * 2;
    // Break if not a boundary cell
    if (flake[flake_idx + 1] != 1) {
        return;
    }
    ...
    if (col > 0 && col < n_cols - 1 && row > 0 && row < n_rows - 1) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            frozen_neighbours += flake[(neighbour_col * n_rows + neighbour_row) * 2];
        }
        ...
        __syncthreads();
        // Update boundary state for neighbours
        for (int i = 0; i < 6; i++) {
            int neighbour_col = neighbours[i][0];
            int neighbour_row = neighbours[i][1];
            int neighbour_idx = (neighbour_col * n_rows + neighbour_row) * 2;
            // Only set unfrozen neighbours to boundary
            if (flake[neighbour_idx] == 0) {
                new_flake[neighbour_idx + 1] = 1;
            }
        }
    }
}
...
def main():
    ...
    flake = np.zeros((flake_size, flake_size, 2)).astype(np.int32)
    # Set the initial state of the flake, with a single frozen cell in the middle
    flake[flake_size // 2, flake_size // 2, 0] = 1
    # Set boundary state of the neighbors of the initial cell to 1
    for neighbour in get_neighbours(flake_size // 2, flake_size // 2):
        flake[neighbour[0], neighbour[1], 1] = 1
    # Set the rule to use
